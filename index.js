...
app.post("/registerMentor", express.json(), async function (request, response) {
  ...
  const result = await client.db("ASM").collection("Mentor").insertOne(data);
...
app.post(
  ...
  async function (request, response) {
    const data = request.body;
    const result = await client.db("ASM").collection("Student").insertOne(data);
    response.send(result);
  }
);
...
app.get("/studentWithoutMentor", async function (request, response) {
  ...
  result.length === 0
    ? response.send({
        msg: "All Students have been assigned to available Mentors.",
      })
    : response.send(result);
});
